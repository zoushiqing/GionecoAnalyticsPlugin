...
class GionecoAnalyticsTransform extends Transform {
    private static Project project
    private GionecoAnalyticsExtension sensorsAnalyticsExtension

    GionecoAnalyticsTransform(Project project, GionecoAnalyticsExtension sensorsAnalyticsExtension) {
    ...
    void _transform(Context context, Collection<TransformInput> inputs, TransformOutputProvider outputProvider, boolean isIncremental) throws IOException, TransformException, InterruptedException {
        ...
        inputs.each { TransformInput input ->
            ...
            input.directoryInputs.each { DirectoryInput directoryInput ->
                ...
                if (dir) {
                    ...
                    dir.traverse(type: FileType.FILES, nameFilter: ~/.*\.class/) {
                        File classFile ->
                            if (GionecoAnalyticsClassModifier.isShouldModify(classFile.name)) {
                                File modified = null
                                if (!sensorsAnalyticsExtension.disableAppClick) {
                                    modified = GionecoAnalyticsClassModifier.modifyClassFile(dir, classFile, context.getTemporaryDir())
                                }
                                if (modified != null) {
            ...
            input.jarInputs.each { JarInput jarInput ->
                ...
                if (!sensorsAnalyticsExtension.disableAppClick) {
                    modifiedJar = GionecoAnalyticsClassModifier.modifyJar(jarInput.file, context.getTemporaryDir(), true)
                }
                if (modifiedJar == null) {
